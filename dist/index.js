  ...
  switch (node.type.toLowerCase()) {
    ...
    case 'block':
      ...
      if (node.mustache.id.string === 'filter') {
        ...
        [node.mustache.params[1], node.mustache.params[node.mustache.params.length === 4 ? 3 : 2]].forEach(function (possiblePropertyNode) {
...
function getCorrespondingSerializer(entityTypeTitle, propertyName) {
  return entitySerializers[entityTypeTitle] && entitySerializers[entityTypeTitle][propertyName] ? entitySerializers[entityTypeTitle][propertyName] : entitySerializers.Base[propertyName];
}
...
module.exports = function exports(config) {
  ...
  this.getTitlePromise = function getTitlePromise(entityTypeTitle, document, parents) {
    ...
    if (entityTypeTitle.substr(-9) === 'Reference' && entityTypeTitle !== 'DocumentReference') {
      ...
      if (document.documentReference && document.documentReference.rootDocumentEntityType) {
        ...
        return this.cbc.getByRef(document.documentReference, parentDocument).then(function (ref) {
          return _this.getTitlePromise.call(_this, entityTypeTitle.substr(0, entityTypeTitle.length - 9), ref, parents);
        }).catch(function () {
    ...
    return _getTitlePromise.call(this, titleFields, entityTypeTitle, document).then(function (titleParts) {
      return composeTitle.call(_this, titleParts, entityTypeTitle, document);
    }).catch(function (err) {
  ...
  this.getPaths = function getPaths(node) {
    ...
    return _.uniq(_getPaths(node).map(function (path) {
      ...
      for (var i = 0; i < nibbles.length; i += 1) {
        var currentNibble = nibbles[i];

        // detect '..'
        var parentRequested = currentNibble === '' && nibbles.length >= i + 2 && nibbles[i + 1] === '' && nibbles[i + 2].indexOf('/') === 0;
