...
describe('#getTemplateData() - Document references', () => {
  it('should work', (done) => {
    cbc.getById(
      'Membership',
      process.env.TEST_MEMBERSHIP_ID
    ).then(membership => factory.getPromise('Membership', membership, template)).then((result) => {
  ...
  it('should be gained in detail', (done) => {
    cbc.getById(
      'Person',
      process.env.TEST_PERSON_ID
    ).then(
      person => factory.getPromiseByPaths('Person', person, ['positions.0.addressReference.address.street'])
    ).then((result) => {
  ...
  it('should support parent referencing', (done) => {
    cbc.getById(
      ...
      person => factory.getPromiseByPaths('Person', person, ['positions.0.emailAddress.emailAddress'])
    ).then((result) => {
      assert.deepEqual(
        result,
        {
  ...
  it('should expose rootDocument when requested', (done) => {
    cbc.getById(
      ...
      person => factory.getPromiseByPaths(
        'Person',
        person,
