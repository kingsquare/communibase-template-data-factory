...
describe("#getTitlePromise()", () => {
  it("Company._title", done => {
    cbc
      ...
      .then(company =>
        factory.getPromise("Company", company, Handlebars.parse("{{_title}}"))
      )
      .then(actual => {
        const expected = {
          _title: "NEVI - Postbus 198 2700 AD ZOETERMEER DE - postal"
        };
  ...
  it("Company._title (with stxt)", done => {
    cbc
      .getById("Company", process.env.TEST_COMPANY_ID)
      .then(company =>
        factoryWithStxt.getPromise(
          "Company",
          company,
          Handlebars.parse("{{_title}}")
        )
      )
      .then(actual => {
        const expected = {
          _title: "NEVI - Postbus 198 2700 AD ZOETERMEER DE - Postadres"
        };
  ...
  it("Debtor._title & Debtor.person._title", done => {
    cbc
      .getById("Debtor", process.env.TEST_DEBTOR_ID)
      .then(debtor =>
        factory.getPromise(
          "Debtor",
          debtor,
          Handlebars.parse("{{_title}}{{person._title}}")
        )
      )
      .then(actual => {
        ...
        const expected = {
          ...
          _title:
            " - NEVI - Postbus 198 2700 AD ZOETERMEER DE - postal J. van Zutphen - " +
  ...
  it("Debtor._title (document reference)", done => {
    cbc
      .getById("Debtor", process.env.TEST_DEBTOR_2_ID)
      .then(debtor => factory.getTitlePromise("Debtor", debtor))
      .then(actual => {
        ...
        assert.deepEqual(
          actual,
          expected,
          "The title is not nicely formatted!"
        );
        done();
      })
      .catch(done);
  ...
  it("Event._title", done => {
    cbc
      ...
      .then(event =>
        factory.getTitlePromise("Event", event).then(actual => {
  ...
  it("Group._title", done => {
    cbc
      .getById("Group", process.env.TEST_GROUP_ID)
      .then(group =>
        factory.getPromise("Group", group, Handlebars.parse("{{_title}}"))
      )
      .then(actual => {
  ...
  it("Invoice._title", done => {
    cbc
      ...
      .then(invoice =>
        factory.getPromise("Invoice", invoice, Handlebars.parse("{{_title}}"))
      )
      .then(actual => {
  ...
  it("Membership._title", done => {
    cbc
      ...
      .then(actual => {
        const expected = {
          _title: "Ooa Professionaliseringsabonnement - J. van Zutphen - 302962"
        };
        assert.deepEqual(actual, expected);
        done();
      })
  ...
  it("Person._title", done => {
    cbc
      ...
      .then(person =>
        factory.getPromise("Person", person, Handlebars.parse("{{_title}}"))
      )
      .then(actual => {
    ...
    cbc
      ...
      .then(person =>
        factory.getPromise(
          "Person",
          person,
          Handlebars.parse("{{positions.0.emailAddressReference._title}}")
        )
      )
      .then(actual => {
        const expected = {
