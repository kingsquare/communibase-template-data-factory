...
module.exports = function exports(config) {
  ...
  this.getEntitiesHashPromise = function getEntitiesHashPromise() {
    if (!this.entitiesHashPromise) {
      this.entitiesHashPromise = this.cbc.getAll('EntityType').then((entities) => {
        const entitiesHash = {};
   ...
   * @returns {Promise}
   */
  this.getPromiseByPaths = function getPromiseByPaths(entityTypeTitle, document, requestedPaths, parents) {
    const serializer = getCorrespondingSerializer(entityTypeTitle, 'getPromiseByPaths');
    return serializer.call(this, entityTypeTitle, document, requestedPaths, parents);
  };

  this.getTitlePromise = function getTitlePromise(entityTypeTitle, document, parents) {
    if (entityTypeTitle.substr(-9) === 'Reference' && entityTypeTitle !== 'DocumentReference') {
      ...
      if (document.documentReference && document.documentReference.rootDocumentEntityType) {
        ...
        return this.cbc.getByRef(document.documentReference, parentDocument).then(
          ref => this.getTitlePromise.call(this, entityTypeTitle.substr(0, entityTypeTitle.length - 9), ref, parents)
        ).catch(() => Promise.resolve('<< Verwijderd >>'));
      }

      // get the subdocument title!
      const subDocument = document[entityTypeTitle[0].toLowerCase() +
    ...
    const _getTitlePromise = getCorrespondingSerializer(entityTypeTitle, '_getTitlePromise');
    const titleFields = getCorrespondingSerializer(entityTypeTitle, 'titleFields');
    const composeTitle = getCorrespondingSerializer(entityTypeTitle, 'composeTitle');

    return _getTitlePromise.call(this, titleFields, entityTypeTitle, document).then(
  ...
  this.getPaths = function getPaths(node) {
    // sanitize paths
    return _.uniq(_getPaths(node).map((path) => {
      ...
      const nibbles = path.replace(/^[\s\uFEFF\xA0.]+|[\s\uFEFF\xA0.]+$/g, '').split('.');
      for (let i = 0; i < nibbles.length; i += 1) {
        let currentNibble = nibbles[i];

        // detect '..'
        let parentRequested = (currentNibble === '') && (nibbles.length >= (i + 2)) && (nibbles[i + 1] === '') &&
