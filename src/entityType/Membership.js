...
module.exports = {
  ...
  composeTitle(chunks, entityTitle, document) {
    ...
    const newChunks = [
      groupName,
      " - ",
      _.isEmpty(personName) ? companyName : personName
    ];
    ...
    return BaseSerializer.composeTitle.apply(this, [
      newChunks,
      entityTitle,
      document
    ]);
    ...
    return BaseSerializer.getPromiseByPaths
      ...
      .then(templateData => {
        if (
          ...
          requestedPaths.indexOf("_active") !== -1
        ) {
          templateData._active = !(
            (document.startDate && new Date(document.startDate) > new Date()) ||
