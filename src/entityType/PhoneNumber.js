...
const formatSubscriberNumber = subscriberNumber => {
  ...
  if (spaceLessNumber.length >= 6) {
    const spacePos =
      subscriberNumber.length <= 7 ? 3 : subscriberNumber.length - 4;
    // just to be sure no double spacing is applied...
    subscriberNumber = `${spaceLessNumber.slice(
      0,
      spacePos
    )} ${spaceLessNumber.slice(spacePos)}`;
  }
...
module.exports = {
  ...
  composeTitle(chunks, entityTypeTitle, document) {
    if (chunks.length > 2) {
      chunks[chunks.length - 3] = formatSubscriberNumber(
    ...
    return BaseSerializer.getPromiseByPaths
      ...
      .then(templateData => {
        if (
          ...
          requestedPaths.indexOf("combined") !== -1
        ) {
