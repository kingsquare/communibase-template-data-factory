    ...
    return BaseSerializer.getPromiseByPaths
      ...
      .then(templateData => {
        ...
        document.invoiceItems.forEach(invoiceItem => {
          ...
          const incltaxMultiplier = taxPercentage
          ...
          if (itemEx !== 0) {
            ...
            totals.taxesRounded[taxPercentage || "null"] += helpers.round(
              taxValue,
              2
            );
            totals.exTaxes[taxPercentage || "null"] += itemEx;
            totals.exTaxesRounded[taxPercentage || "null"] += helpers.round(
              itemEx,
              2
            );
          }
          ...
          totals.in +=
            itemEx * (document.reverseChargedVat ? 1 : incltaxMultiplier);
          totals.inRounded +=
            itemExRounded + (document.reverseChargedVat ? 0 : taxValueRounded);
        });
        ...
        _.each(["taxes", "taxesRounded"], taxSumType => {
          ...
          const requestedTaxesVariables = helpers.getRequestedSubVariables(
            requestedPaths,
            `${taxSumType}.#`
          );
          if (requestedTaxesVariables.length !== 0) {
            ...
            _.each(totals[taxSumType], (value, taxPercentage) => {
              ...
              if (
                requestedTaxesVariables.indexOf("#") !== -1 ||
                requestedTaxesVariables.indexOf("percentage") !== -1
              ) {
                tax.percentage = taxPercentage;
              }
              if (
                requestedTaxesVariables.indexOf("#") !== -1 ||
                requestedTaxesVariables.indexOf("value") !== -1
              ) {
                tax.value = value;
              }
              if (
                requestedTaxesVariables.indexOf("#") !== -1 ||
                requestedTaxesVariables.indexOf("total") !== -1
              ) {
        ...
        if (
          ...
          requestedPaths.indexOf("isCredit") !== -1
        ) {
